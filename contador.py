...
if __name__ == "__main__":
    ...
    words = sc.textFile("gs://desafio-dataproc-junior2021/livro.txt").flatMap(lambda line: line.split(" "))
    wordCounts = words.map(lambda word: (word, 1)).reduceByKey(lambda a,b:a +b).sortBy(lambda a:a[1], ascending=False)
    wordCounts.saveAsTextFile("gs://desafio-ataproc-junior2021/resultado")